<template>
  <div class="container mt-2 mb-2">
    ...
    <gl-form @submit="createNetworkInterface">
      <gl-form-group
        id="name-input-group"
        label="Name:"
...
import EC2Client, { CreateNetworkInterfaceRequest } from "aws-sdk/clients/ec2";
...
export default class NewNetworkInterface extends DaintreeComponent {
  form = {
    region: "",
    vpcId: "",
    subnetId: "",
  ...
  async createNetworkInterface(evt: Event): Promise<void> {
    evt.preventDefault();

    this.hideErrors("createNetworkInterface");
    ...
    const params: CreateNetworkInterfaceRequest = {
    ...
    try {
      ...
      const data = await EC2.createNetworkInterface(params).promise();
      const verb =
        data.NetworkInterface?.Status?.toLowerCase() === "available"
          ? "Created"
          : "Creating";
      ...
      this.showAlert({
        ...
        key: "creatingNetworkInterface",
        resourceId: data.NetworkInterface?.NetworkInterfaceId,
      });
      this.$router.push("/network/interfaces");
    } catch (err) {
      this.showError(err.message, "createNetworkInterface");
    } finally {
